...
void HudRenderer::drawSpeedLimitSigns(QPainter &p, const QRect &surface_rect) {
  ...
  // Calculate sign dimensions based on whether we have sub-text
  // Remove unused variable since we're now using consistent height
  ...
  if (false) {
    ...
    if (!slcSubText.isEmpty()) {
      ...
      int offset_circle_size = 55;  // Slightly bigger circle size
      QRect offset_circle_rect(
        circle_rect.right() - offset_circle_size/2 - 3,  // Position overlapping right edge
        circle_rect.top() + offset_circle_size/2 - 3,    // Position overlapping top edge
        offset_circle_size,
        offset_circle_size
      );

      // Draw black background circle for offset
      p.setPen(Qt::NoPen);
      p.setBrush(QColor(0, 0, 0, 255)); // Black background
      p.drawEllipse(offset_circle_rect);

      // Draw white border for offset circle
      p.setPen(QPen(QColor(255, 255, 255, 255), 2));
      p.setBrush(Qt::NoBrush);
      p.drawEllipse(offset_circle_rect);

      // Draw offset text in white
      p.setFont(InterFont(17, QFont::Bold));
      p.setPen(QColor(255, 255, 255, 255)); // White text
